...
def preprocess(args):
    ...
    token_ids_array = np.memmap(
        ...
        shape=(all_linecnt, args.max_doc_length), mode='w+', dtype=np.int32)
    token_length_array = []
    ...
    meta = {
        ...
        'embedding_size': args.max_doc_length}
...
def PassagePreprocessingFn(args, line, tokenizer):
    ...
    passage_len = min(len(passage), args.max_doc_length)
    input_id_b = pad_input_ids(passage, args.max_doc_length)
